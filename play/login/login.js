...
//   if (!("gapi" in window))
//   { console.error ("No gapi in window");
//     return; }
//   gapi.load('auth2', () =>
//             { gapi.auth2.init().then(
//                 () =>
//                     { console.log("Got Google login success response");
//                       Tootsville.login.googleSignIn(); },
//                 (err) =>
//                     { console.log("Got Google login error response");
...
{ console.log("Switch Google Account: TODO");
  // if (window.gapi)
  // { console.log("Using gapi");
  //   Tootsville.login.googleSignOut().then(Tootsville.login.googleInit); } else if (window.googleyolo)
  // { console.log("Using Yolo");
  //   Tootsville.login.googleYoloSignIn(window.googleyolo); } else
  // { console.error ("Neither gapi nor yolo found"); }
}
...
{ return new Promise( (finish, reject) =>
                      { Tootsville.login.rest('users','/users/me/toots',
                                              { want: [ 'avatar', 'name', 'note', 'child-p', 'sensitive-p' ],
                                                order: 'last-seen' }).then( response =>
                                                                            { if (response.toots.length == 0)
                                                                              { reject(); } else
                                                                              { finish(response.toots); } }, Tootsville.login.criticalFailure); }); }
...
{ googleyolo.retrieve(
    { supportedAuthMethods:
      ...
      suppertedIdTokenProviders:
      [{ uri: "https://accounts.google.com",
         /* TODO move Google clientID someplace cooler */
         clientId: "1052481910502-kb8u1evb6c82dl2o5km4vu6q5mmudkfo.apps.googleusercontent.com" }],
      context: "signIn" /* or "continue" */ }); };
